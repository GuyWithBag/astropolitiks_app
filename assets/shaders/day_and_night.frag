...
// Shader taken from https://www.shadertoy.com/view/ltlSWB at 2025-10-15, Created by BitOfGold in 2016-09-09
// License: None provided, refer to https://www.shadertoy.com/terms:
// "We recommend you paste your preferred license on top of your code, if you don't place a license on a shader, it will be protected by our default license:
// This work is licensed under a [Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.](https://creativecommons.org/licenses/by-nc-sa/3.0/deed.en)"

// Any lines between "// <SHADER>" and "// </SHADER>" are copy-pasted directly, without modification, from the original shader code found within the aforementioned shadertoy link. Any code outside This includes any comments by the original author.
...
const int steps = 64; //16 is fast, 128 or 256 is extreme high
// <SHADER>
...
const float DEGRAD = M_PI / 180.0;
    ...
    float cloudy = 1.0; //0.6 //0.0 clear sky
#else
// </SHADER>
...
#endif
// </SHADER>
// The following functions are an attempt to replicate the texture sampling functions in shadertoy.
// Code taken from ChatGPT; may be faulty.
// None of the original shader code was used in any prompts.
vec2 iChannelResolution(int channel) {
    if (channel == 0) {
        return iChannelResolution0;
    } else {
        return vec2(0); // fallback resolution (zero)
    }
}

// wraps the uv around the channel
vec4 textureIChannelNearest(int channel, vec2 uv) {
    uv = fract(uv);
    if (channel == 0) {
        return texture(iChannel0, uv);
    } else {
        return vec4(1.0, 0.0, 1.0, 1.0); // fallback color (magenta)
    }
}

// imperfect linear interpolation, i think
vec4 textureIChannel(int channel, vec2 uv) {
    vec2 res = iChannelResolution(channel);

    // Convert to texel space and get interpolation fraction
    vec2 texelPos = uv * res - 0.5;
    vec2 f = fract(texelPos); // interpolation weights

    vec2 i0 = mod(floor(texelPos), res);
    vec2 i1 = mod(i0 + vec2(1.0, 0.0), res);
    vec2 i2 = mod(i0 + vec2(0.0, 1.0), res);
    vec2 i3 = mod(i0 + vec2(1.0, 1.0), res);
    ...
    // Fetch texels
    vec4 c0 = textureIChannelNearest(channel, uv0);
    vec4 c1 = textureIChannelNearest(channel, uv1);
    vec4 c2 = textureIChannelNearest(channel, uv2);
    vec4 c3 = textureIChannelNearest(channel, uv3);

    // Bilinear interpolation
    vec4 cx0 = mix(c0, c1, f.x);
    vec4 cx1 = mix(c2, c3, f.x);
...
// </SHADER>
	...
	vec2 rg = textureIChannel( 0, (uv+ 0.5)/256.0).yx;
	// vec2 rg = textureIChannel( 0, (uv+ 0.5)/256.0).yx;
// <SHADER>
...
vec4 generate(in vec2 uv, in vec2 fragCoord, in vec2 sunpos, in float t) {
    ...
    void main() {
// </SHADER>
        vec2 vUV = FlutterFragCoord() / iResolution;
// <SHADER>
...
// </SHADER>
        ...
        outFragColor = generate(uv,uv,sunpos,t);
// <SHADER>
    }
#endif
// </SHADER>